...
if (COption::GetOptionString("main", "new_user_registration", "N") == "N" && $_GET['b'] != 'Y')
...
if($_REQUEST['UF_GROUP'] == 15)
{
	$arDefaultFields[] = 'PERSONAL_GENDER';
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	...
	if($_REQUEST['conditions'] != 'Y')
	{
		$arResult["ERRORS"]["conditions"] = GetMessage("REGISTER_FIELD_REQUIRED");
	}
...
if (is_array($arUserFields) && count($arUserFields) > 0)
{
	...
	foreach ($arUserFields as $FIELD_NAME => $arUserField)
	{
		...
		if($FIELD_NAME == 'UF_STATUS' && $_REQUEST['UF_GROUP'] == 14)
		{
			if (intval($_REQUEST[$FIELD_NAME]) == 0)
				$arResult["ERRORS"][$FIELD_NAME] = GetMessage("REGISTER_FIELD_REQUIRED");
		}

		if($FIELD_NAME == 'UF_WED_DATE' && $_REQUEST['UF_GROUP'] == 15)
		{
			if (intval($_REQUEST[$FIELD_NAME]) == 0)
				$arResult["ERRORS"][$FIELD_NAME] = GetMessage("REGISTER_FIELD_REQUIRED");
		}
