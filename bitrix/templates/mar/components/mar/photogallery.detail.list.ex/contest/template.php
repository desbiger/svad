...
$current_date = "";
?>
<div class="album-container photo-items-list photo-photo-list" id="photo_list_<?= htmlspecialcharsEx($arParams["~UNIQUE_COMPONENT_ID"])?>">
...
foreach ($arResult["ELEMENTS_LIST"] as $key => $arItem)
{
	...
	$arItem["TITLE"] = htmlspecialcharsEx(TruncateText(strip_tags($arItem["~PREVIEW_TEXT"]), 20));
	...
	<div id="photo_cont_<?=$arItem["ID"]?>" class="photo-item-cont <?if ($arParams["PERMISSION"] >= "X"){echo ' photo-item-cont-moder';}?>" title="<?= $arItem["TITLE"]?>">
		<div class="pic_album" style="width: <?= $arParams['THUMBNAIL_SIZE']?>px; height: <?= $arParams['THUMBNAIL_SIZE']?>px;"  id="photo_<?=$arItem["ID"]?>">
			<a class="album_pic photo-item-inner" href="<?=$arItem["URL"]?>">
				<img src="<?= $src?>" border="0" style="<?= $item_w?> <?= $item_h?> <?= $item_left?> <?= $item_top?>;" />
        	<span class="info">
          	<b><?=intval($arItem['PROPERTIES']['FORUM_MESSAGE_CNT']['VALUE'])?> <?=GetMessage("P_COMMENTS")?></b>
            <span class="like"><?=floor($arItem['PROPERTIES']['rating']['VALUE'])?></span>
            <span class="look"><?=intval($arItem['SHOW_COUNTER'])?></span>
          </span>
			</a>
			<?if($arParams["PERMISSION"] == "W" && $arParams['USER']['GROUP_PROF']):?>
			<a href="#prof" class="potr<?=$arItem['PROPERTIES']['IS_PORTFOLIO']['VALUE']?' act':''?>" data-id="<?=$arItem["ID"]?>"><?= GetMessage("P_PORTFOLIO_SMALL")?></a>
			<?endif;?>
		</div>
	</div>
	<?
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.pic_album .potr').click(function(){
			$(this).toggleClass('act');
			if($(this).hasClass('act'))
			{
				$.post("/portfolio/", {action: 'add', id: $(this).data('id') } );
			}
			else
			{
				$.post("/portfolio/", {action: 'drop',  id: $(this).data('id') } );
			}
			return false;
		});
	});
