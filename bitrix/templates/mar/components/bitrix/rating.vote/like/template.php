...
<span class="ilike-light">
	<span	class="bx-ilike-button <?=($arResult['VOTE_AVAILABLE'] == 'Y' ? '' : 'bx-ilike-button-disable')?>" id="bx-ilike-button-<?=CUtil::JSEscape(htmlspecialchars($arResult['VOTE_ID']))?>">
		<span class="bx-ilike-right-wrap <?=($arResult['USER_HAS_VOTED'] == 'N' ? '' : 'bx-you-like')?>">
			<span class="bx-ilike-right"><?=htmlspecialcharsEx($arResult['TOTAL_VOTES'])?>
			</span>
		</span>
		<span class="bx-ilike-left-wrap" <?=($arResult['VOTE_AVAILABLE'] == 'Y' ? '' : 'title="' . $arResult['ALLOW_VOTE']['ERROR_MSG'] . '"')?>><?
	if ($arResult['VOTE_AVAILABLE'] == 'Y'):  ?>
		<a href="#like"
			 class="bx-ilike-text"><?=($arResult['USER_HAS_VOTED'] == 'N' ? CUtil::JSEscape(htmlspecialchars($arResult['RATING_TEXT_LIKE_Y'])) : CUtil::JSEscape(htmlspecialchars($arResult['RATING_TEXT_LIKE_N'])))?>
		</a>
	<?endif;?>
		</span>
	</span>
