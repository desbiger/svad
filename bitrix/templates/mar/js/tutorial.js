function fb_sharer_init(){
	$('.fb_sharer').each(function(){
		...
		_this.html('<a href="http://www.facebook.com/sharer.php?u='+pageuri+'&src=sp" target="_blank" class="fb_sharer_btn">Нравится</a>\
			<span>0</span>').find('.fb_sharer_btn').click(function(){
				var social_box = $(this).parent();
		...
		$.getJSON('http://api.facebook.com/restserver.php?method=links.getStats&callback=?&urls=' + pageuri + '&format=json', function(data) {
			$('span', _this).text(data[0].share_count);
		});
...
$(function(){
	...
	$('.on-favorit, .favorit-delete').click(function(){
		$(this).toggleClass('selected');
		if($(this).hasClass('selected'))
		{
			$.post("/favorite/", {action: 'add', id: $(this).data('id') } );
		}
		else
		{
			$.post("/favorite/", {action: 'drop',  id: $(this).data('id') } );
		}
		return false;
	});
	fb_sharer_init();
